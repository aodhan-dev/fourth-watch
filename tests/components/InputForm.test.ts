...
import InputFormHarness from './InputFormHarness.svelte';
...
describe('InputForm', () => {
  it('renders without crashing', () => {
    render(InputFormHarness, { initial: defaultValue, onRoll: vi.fn(), canRoll: false });
  ...
  it('Roll button is enabled when canRoll is true', () => {
    render(InputFormHarness, { initial: defaultValue, onRoll: vi.fn(), canRoll: true });
    const btn = screen.getByRole('button', { name: /roll/i }) as HTMLButtonElement;
  ...
  it('mode radio inputs are present and respond to change', async () => {
    const { container } = render(InputFormHarness, {
